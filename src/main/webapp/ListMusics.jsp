...
<div class="container">
    ...
    <button id="showMusicsBtn">Show All Musics in the Database</button>
    <table id="musicTable">
        <thead>
            <tr>
                <th>ID</th>
...
<script>
document.getElementById("showMusicsBtn").addEventListener("click", function() {
    // This attaches a click event listener to the element with the ID "showMusicsBtn".
    // When this button is clicked, the function inside the event listener will execute.

    fetch("/musicpage/ListOfMusics")
    ...
    .then(data => {
        ...
        data.forEach(music => {
            ...
            let aid = music.id;
            let aname = music.name;
            let adescription = music.descrip;
           // let acontentlink = music.contentLink;
             let acontentlink = "/musicpage/music/"+ aname;
             //alert(acontentlink);
            ...
            row.innerHTML = '<td>' + aid + '</td>' +
                            '<td>' + aname + '</td>' +
                            '<td>' + adescription + '</td>' +
                            '<td><span class="music-icon" data-url="' + acontentlink + '"> &#127911 </span> <span class="downloadButton" style="cursor: pointer;">  &#x1F4E5;</span></td>';
            //<img src=/GIFS_Webpage/MusicRelated/Sound - Animated Icon.gif"  alt="Animated Emoji" style="width: 16px; height: 16px;">
            ...
            icon.addEventListener('click', function() {
                ...
                .then(blob => {
                    ...
                    newTab.document.write(
                        ...
                        '<body style="margin: 0;">' +
                        '<audio controls style="width: 100%; height: 100vh;">' +
         ...
         // Add event listener to download buttons
                document.querySelectorAll('.downloadButton').forEach(downloadButton => {
                    downloadButton.addEventListener('click', function(event) {
                        ...
                        const musicUrl = this.parentNode.querySelector('.music-icon').getAttribute('data-url');
...
<!-- Add the GIF image -->
    <a href="/ListVideos">
        <img src="GIFS_Webpage/VideoRelated/TomandFriend Playing Guitar.gif" class="enlarge-gif" alt="Enlarge" title="Click to redirect to Video Database">
    </a>
</body>
</html>
